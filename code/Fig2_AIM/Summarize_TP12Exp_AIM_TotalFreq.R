...
Extract <- function(file.name, dir.input, FirstSecondType, ThirdType, condition){
  ...
  for(aim in AIMType){
    d_aim <- dplyr::filter(d, class == aim)
    ...
    for(i in 1:length(FirstSecondType)){
      ...
      dsub <- dplyr::filter(d_aim, Resp == fst)
  ...
  output.all$Resp <- FirstSecondType
  output.all$sample <- file.name
...
name.output <- "result/Fig2/AIM.FirstSecond.Freq.csv"

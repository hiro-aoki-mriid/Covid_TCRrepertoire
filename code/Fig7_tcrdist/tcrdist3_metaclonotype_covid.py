...
def find_metaclonotypes(
    ...
    assert os.path.isfile(os.path.join(source_path, antigen_enriched_file)) #ファイルがあるかどうかの確認
	...
	# This mix balances dense sampling of sequences near the
    ...
    # Methods for details about inverse probability weighting adjustment). 
	
	# Using this approach we are able to estimate the abundance of TCRs similar to a centroid
    # TCR in an unenriched background repertoire of ~1,000,000 TCRs, using a
    # comparatively modest background dataset of 200,000 TCRs. 
	
	# While this estimate may underestimate the true specificity, since some of the neighborhood TCRs in
	...
	#Output: AER: antigen enriched repertoire / BUR: unenriched repertoire
    ...
    # Many of meta-clonotypes contain redundant information. (metaclonotypeの重複を削除)
    # We can winnow down to less-redundant list. We do this by ranking clonotypes from most to least specific. 
        # <min_nsubject> is minimum publicity of the meta-clonotype,  
        # <min_nr> is minimum non-redundancy
    # Add neighbors, K_neighbors, and nsubject columns (neighborに含まれるクローン数)
    ...
    # We determine how many <nsubjects> are in the set of neighbors (個体数)
...
if __name__ == "__main__":
    ...
    path = 'mydata/result/intermediate/7_tcrdist/Input.tcrdist3'
    ...
    for file in all_files:
        print(file)
        find_metaclonotypes(project_path = "mydata/result/intermediate/7_tcrdist/hla_restricted_meta_clonotypes",
                            source_path = path,

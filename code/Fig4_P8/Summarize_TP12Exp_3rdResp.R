...
Types <- c("2nd_3rd", "AIM_2nd", "2nd") 
freq_params <- c("TP2", "TP3", "TP6", "TP8")
count_params <- c("TP2TP3dif", "TP6TP8dif")
...
Extract <- function(file.name, dir.input, FirstSecondType, ThirdType){
  ...
  output <- createEmptyDf(length(Types), length(all_params), colnames = all_params)

  for(i in 1:length(Types)){
    type <- Types[i]
    if(type == "2nd"){
      dsub <- dplyr::filter(d, Resp == "2nd")
    }
    if(type == "2nd_3rd"){
      dsub <- dplyr::filter(d, Resp == "3rd" | Resp == "2nd")
    }
    if(type == "AIM_2nd"){
      dsub <- dplyr::filter(d, Resp == "2nd" & class == "AIM")
    }
    
    #Calculate total freq. or freq. diff
    dsub_freq <- dplyr::select(dsub, freq_params)
    output[i, 1:(length(freq_params))] <- apply(dsub_freq, 2, sum)
    
    #Calculate count of expanding clones
    dsub_exp <- dplyr::select(dsub, count_params)
    count_exp <- function(vec){sum(vec == "Exp")}
    output[i, (1+length(freq_params)):length(c(freq_params, count_params))] <- apply(dsub_exp, 2, count_exp)
    ...
    if(TP3_count > 9 & TP8_count >9){
      ...
      output[i, (1+length(c(freq_params, count_params))):length(all_params)] <- c(pi_TP3, pi_TP8, de_TP3, de_TP8)
    } else {
      output[i, (1+length(c(freq_params, count_params))):length(all_params)] <- rep("NA", 4)
    }
  }
  output$Type <- Types
...
name.output <- str_c(dir.output, "SecondResp_TP2vsTP8.csv", sep = "/")

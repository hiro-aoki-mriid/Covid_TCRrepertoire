...
#Input layer
dir.input <- "Francis_hla_restricted_meta_clonotypes"
...
Main <- function(file.name, dir.input, dir.output){
  ...
  if(nrow(table) > 0){
    ...
    protein <- str_remove(Epitope.inf[4], pattern = "SARS2_")
...
names(out) <- c("set", "sars_cov_2_genomic_position", "protein_coordinate", "protein",
                "feature", "cdr3_b_aa", "v_b_gene", "j_b_gene", "pgen", "radius", "regex")
name.output <- str_c(dir.output, "Francis_metaclonotypes.tsv", sep = "/")
